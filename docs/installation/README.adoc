...
----
$ sudo docker-compose up -d sakai_mysql
----

MySQLの起動完了を確認する。

----
$ sudo docker logs -f sakai_mysql
...
----
$ sudo docker-compose up -d sakai
----

http://localhost:8888/portal からSakaiにアクセスできることを確認する。
アクセス情報はSakai-LMS-LAファイルに記載されている。
...
----
$ sudo docker-compose up -d superset
----

Sakaiのステートメントを取り扱うため、外部テーブルを変更し、カラムを追加・リネームする。

----
$ sudo docker-compose exec superset-db psql learninglocker -c '
...
https://github.com/RCOSDP/LAaaS-docker/blob/release-v3.0/README.adoc#48-superset%E3%81%AE%E8%A8%AD%E5%AE%9A[Supersetの設定手順]の4.8.1〜4.8.4に従い、 `xapi_statements` テーブルをSupersetに追加する。
...
https://github.com/RCOSDP/LAaaS-docker/blob/release-v3.0/README.adoc#49-jupyterhub%E3%81%AE%E8%A8%AD%E5%AE%9A[JupyterHubの設定]に従いログインし設定を行う。添付されているノートブックは analyze-xapi-statements.ipynb のみとなっていることに注意。
...
----
$ sudo docker-compose up -d dummy-xapi
$ sudo docker-compose exec dummy-xapi npm start
----
